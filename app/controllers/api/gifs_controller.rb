class Api::GifsController < ApplicationController
  def index
    ...
    # @gifs = HTTP.headers("Authorization" => "Bearer #{ENV['GIPHY_API_KEY']}").get("https://api.giphy.com/v1/gifs/search?q=#{params[:search]}&limit=25&offset=0&rating=G&lang=en").parse["data"]
    # render 'index.json.jb'
  end
end
